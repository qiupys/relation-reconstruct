...
parser.add_argument('--device', default=2, type=int, help='cuda visible device id')
...
from models import Server, MLPWorker
...
for partition in range(1, 10):
    ...
    for rep in range(args.repeat):
        setupSeed(10 * rep)
        data, split_data, num_data, num_classes = loadData(name=args.dataset, partition=partition * 0.1, device=device)
        workers = [MLPWorker(num_node_features=len(split_data[0].x[0]), num_embed=args.ndim).to(device),
                   MLPWorker(num_node_features=len(split_data[1].x[0]), num_embed=args.ndim).to(device)]
    ...
    path = './results/{}/attackSceneZero/'.format(args.dataset)
    if not os.path.exists(path):
        os.mkdir(path)
    np.savez('./results/{}/attackSceneZero/partition_{}'.format(args.dataset, partition), np.array(par_results))
