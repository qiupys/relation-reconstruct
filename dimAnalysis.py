...
from models import MLPWorker, GCNWorker, Server
...
for d in range(3, 8):
    ...
    for rep in range(args.repeat):
        ...
        workers = [MLPWorker(num_node_features=len(split_data[0].x[0]), num_embed=ndim).to(device),
                   GCNWorker(in_features=len(split_data[1].x[0]), out_features=ndim).to(device)]
        server = Server(num_party=args.nparty, num_embed=ndim, num_classes=num_classes).to(
            device)

        print('{} th, ndim: {}, dataset: {}, VFL training...'.format(rep, ndim, args.dataset))
        eval_results = []
        ...
        start = time.clock()
        ap_tar = recEncEmbed(v_adv, v_top, 2, num_data, ndim, server, device)
    ...
    if not os.path.exists(path):
        os.mkdir(path)
    np.savez('./results/{}/sensitivity/dim_{}'.format(args.dataset, ndim), np.array(results))
