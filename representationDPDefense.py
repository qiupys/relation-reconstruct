...
from utils import setupSeed, calEmbeds, calAUC, recEncEmbed, loadData

device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')


def vflTrain(workers, server, split_data, data, num_data, scale):
    ...
    noise = torch.FloatTensor(np.random.normal(0, scale=scale, size=[num_data, args.ndim])).to(device)
...
for scale in range(1, 10):
    ...
    for rep in range(args.repeat):
        ...
        print('{} th, scale: {}, dataset: {}, VFL training...'.format(rep, scale, args.dataset))
        eval_results = []

        # training result
        for epoch in range(1, 301):
            train_acc, val_acc, test_acc = vflTrain(workers, server, split_data, data, num_data, scale * 0.1)
    ...
    path = './results/{}/representationDP/'.format(args.dataset)
    if not os.path.exists(path):
        os.mkdir(path)
    np.savez('./results/{}/salaDP/scale_{}'.format(args.dataset, scale), np.array(mp_results))
